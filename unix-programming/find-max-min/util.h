...
/* It returns the Result object that is store in the given File.
...
/* It returns the Result object that is in the given string.
   ...
   "%d %d" The first parameter is minimum, the second parameter is maximum.
*/
Result getResultsFromString(char *string);
 ...
 */
void executeTemplate(void (*createProcess)(int, Pair [], int []),
		int seed, int arraysize, int nprocess);

/** It prints a message. The message is not saved in the buffer.*/
void printfMessage(char *msg);
